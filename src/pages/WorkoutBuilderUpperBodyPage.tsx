...
export default function WorkoutBuilderUpperBodyPage() {
  const { t } = useLanguage()
  ...
  const groups = getWorkoutBuilderGroups(isGymRoute ? 'gym' : 'home', 'fat-loss', 'upper-body')

  return (
    <ExerciseBuilderScreen
      title={t('workoutBuilder.upperBodyBuilderTitle')}
      subtitle={t('workoutBuilder.upperBodyBuilderSubtitleFatLoss')}
      backTo={builderPath}
      groups={groups}
      trainingPath={trainingPath}
      storageKey={`${isGymRoute ? 'gym' : 'home'}-fat-loss-upper-body`}
      modePreset="fat-loss"
      useGuidedSplitFlow
    />
  )
