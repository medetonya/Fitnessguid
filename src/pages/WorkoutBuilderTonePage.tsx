import { ArrowLeft, ChevronRight, House, Layers, SlidersHorizontal } from 'lucide-react'
...
const getCardIcon = (icon: StructureCard['icon']) => {
  if (icon === 'full') {
...
export default function WorkoutBuilderTonePage() {
  const { t } = useLanguage()
  const { pathname } = useLocation()
  const isGymRoute = pathname.includes('/workouts/gym/')
  const builderBasePath = isGymRoute ? '/workouts/gym/goal/muscle-tone/workout-builder' : '/workouts/home/goal/muscle-tone/workout-builder'
  ...
  const structureCards: StructureCard[] = [
    ...
    {
      title: t('workoutBuilder.upperBodyTitle'),
      subtitle: t('workoutBuilder.upperBodySubtitle'),
      icon: 'split',
      borderClass: 'border-[#d4d4d8]',
      titleClass: 'text-gray-900',
      to: `${builderBasePath}/upper-body`,
    },
    {
      title: t('workoutBuilder.lowerBodyTitle'),
      subtitle: t('workoutBuilder.lowerBodySubtitle'),
      icon: 'split',
      borderClass: 'border-[#d4d4d8]',
      titleClass: 'text-gray-900',
      to: `${builderBasePath}/lower-body`,
    },
  ]
    ...
    <div className="min-h-screen bg-[#f5f5f5]">
      <main className="mx-auto max-w-5xl px-6 py-8 md:py-10">
        ...
        <section className="mb-8">
          ...
          <h1 className="font-serif text-6xl leading-[0.95] text-gray-900 md:text-7xl">{t('workoutBuilder.toneTitle')}</h1>
          <p className="mt-4 text-2xl text-gray-700">{t('workoutBuilder.toneSubtitle')}</p>
        </section>
